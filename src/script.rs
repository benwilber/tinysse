...
impl Script {
    ...
    pub async fn publish(&self, pub_req: PublishRequest) -> anyhow::Result<Option<PublishRequest>> {
        if let Ok(func) = self.lua.named_registry_value::<mlua::Function>("publish") {
            if let Some(pub_req) = func.call_async::<Option<PublishRequest>>(pub_req).await? {
                Ok(Some(pub_req))
            } else {
                Ok(None)
            }
    ...
    pub async fn subscribe(
        ...
        if let Ok(func) = self.lua.named_registry_value::<mlua::Function>("subscribe") {
            if let Some(sub_req) = func.call_async::<Option<SubscribeRequest>>(sub_req).await? {
                Ok(Some(sub_req))
            } else {
                Ok(None)
            }
    ...
    pub async fn catchup(
        ...
        if let Ok(func) = self.lua.named_registry_value::<mlua::Function>("catchup") {
            return Ok(func
                .call_async::<Option<Vec<Message>>>((sub_req, last_event_id))
                .await?);
        }
